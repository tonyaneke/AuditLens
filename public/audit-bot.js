...
function isPrimaryOwner(o){ return !!(window.AMS_USER && o && o.ownerUserId && o.ownerUserId===window.AMS_USER.id); }
function isSecondaryOwner(o){ return !!(window.AMS_USER && o && o.secondaryOwnerUserId && o.secondaryOwnerUserId===window.AMS_USER.id); }
function ownerEmailFor(userId){ if(!userId) return ""; const u=(_directoryCache||[]).find(x=>x.id===userId); if(u&&u.email) return u.email; const d=departments().find(x=>x.headUserId===userId); return (d&&d.headEmail)||""; }
...
function viewTracker(){
  ...
  const recentClosed=obs.filter(o=>(o.status||"Open")==="Closed").sort((a,b)=>String(b.closedDateISO||"").localeCompare(String(a.closedDateISO||"")));
...
function createAuditPlan(){
  ...
  if(planYearHasData(y)) uiConfirm(`A plan already exists for ${y}. Switch to it and re-apply the selected units?`,apply,{confirmLabel:"Switch & apply"});
  else apply();
}
...
function viewFraud(){
  ...
  h+=`<div class="card"><div class="row" style="align-items:center"><div class="seclabel" style="margin:0">Fraud Prevention Plan</div><div class="spacer"></div>
    ...
    ${plan?`<div class="txt" style="white-space:pre-wrap;margin:8px 0 4px;padding:10px 12px;background:#f8fafc;border:1px solid var(--line);border-radius:8px${fraudPlanExpanded?"":";max-height:96px;overflow:hidden;position:relative;-webkit-mask-image:linear-gradient(#000 60%,transparent)"}">${esc(plan)}</div>
    ${plan.length>260?`<button class="btn ghost sm" style="margin-bottom:8px" onclick="fraudPlanExpanded=!fraudPlanExpanded;render()">${fraudPlanExpanded?"▲ Show less":"▼ Read more"}</button>`:""}`:`<div class="hint" style="margin:8px 0">No plan overview yet.</div>`}
...
function modalFraud(id){
  if(!requireHead())return;
  const F=fraudList(); const isEdit=!!id;
  const f=isEdit?F.find(x=>x.id===id):(_newFraud||{year:String((new Date()).getFullYear()),process:"",category:"Asset Misappropriation",scheme:"",description:"",likelihood:3,impact:3,existingControls:"",controlStrength:"Moderate",residualOverride:"",preventionAction:"",owner:"",status:"Identified"});
  openModal(isEdit?"Edit fraud risk":"Add fraud risk — step 1 of 2: risk details",`
    <div class="f3">
    ...
    <label>Description (how the fraud could occur)</label><textarea id="fr_desc" oninput="autoGrow(this)">${esc(f.description)}</textarea>
    <div class="f3">
...
/* ---- Add-fraud step 2: AI-proposed preventive actions from likelihood/impact/control strength ---- */
...
function openFraudStep2(generate){
...
Existing anti-fraud controls: ${_newFraud.existingControls||"(none noted)"}
Likelihood: ${_newFraud.likelihood}/5 (${LIKE_LABEL[_newFraud.likelihood]}) · Impact: ${_newFraud.impact}/5 (${IMP_LABEL[_newFraud.impact]})
Control strength: ${_newFraud.controlStrength} · Residual risk: ${res}
...
function renderNewFraudActions(){
  const el=document.getElementById("nfa_wrap"); if(!el) return;
  el.innerHTML=`
    ${_newFraudRationale?`<div class="hint" style="margin-bottom:10px">✦ ${esc(_newFraudRationale)}</div>`:""}
    ${_newFraudActions.length?_newFraudActions.map((a,i)=>`<div class="obs-block" style="padding:10px 12px;margin-bottom:10px">
      <label>Prevention / response action</label><textarea id="nfa_text_${i}" style="min-height:48px" oninput="autoGrow(this)">${esc(a.text)}</textarea>
      <div class="f3" style="margin-top:6px">
        <div><label>Control type</label><select id="nfa_type_${i}">${ACTION_TYPES.map(t=>`<option${a.type===t?" selected":""}>${t}</option>`).join("")}</select></div>
        <div><label>Target date</label><input id="nfa_tgt_${i}" value="${esc(a.targetDate||"")}" placeholder="e.g. Q4 2026"></div>
        <div><label>Status</label><select id="nfa_status_${i}">${ACTION_STATUS.map(s=>`<option${a.status===s?" selected":""}>${s}</option>`).join("")}</select></div>
      </div>
      <div class="row" style="margin-top:6px"><div class="spacer"></div><button class="btn ghost sm danger" type="button" onclick="removeNewFraudAction(${i})">Remove</button></div>
    </div>`).join(""):`<div class="note" style="margin-bottom:10px"><b>No preventive action needed.</b><div class="hint" style="margin-top:4px">${esc(_newFraudRationale||"The residual risk is acceptably low given existing controls.")} You can still add actions manually below.</div></div>`}
    <div class="row" style="gap:8px">
      <button class="btn sec sm" type="button" onclick="addNewFraudActionRow()">＋ Add action</button>
      <button class="btn ghost sm ai-generate-btn" type="button" onclick="syncNewFraudActions();generateNewFraudActions()">↻ Regenerate with AI</button>
    </div>`;
}
function syncNewFraudActions(){
  _newFraudActions=_newFraudActions.map((a,i)=>({
    text:val("nfa_text_"+i)||a.text,type:val("nfa_type_"+i)||a.type,
    targetDate:val("nfa_tgt_"+i),status:val("nfa_status_"+i)||a.status
  })).filter(a=>a.text);
}
function addNewFraudActionRow(){ syncNewFraudActions(); _newFraudActions.push({text:"",type:"Preventive",targetDate:"",status:"Planned"}); renderNewFraudActions(); }
function removeNewFraudAction(i){ syncNewFraudActions(); _newFraudActions.splice(i,1); renderNewFraudActions(); }
async function fraudFinishAdd(){
  syncNewFraudActions();
  const btn=document.getElementById("nfa_add"); const done=btnBusy(btn,"Adding…");
  const obj={id:uid(),..._newFraud,createdAt:new Date().toISOString(),
...
function modalReadyForClosure(aid,rid,oid){
  const o=findObs(aid,rid,oid); if(!o) return;
  if(!isPrimaryOwner(o)&&!isHeadUser()){ toast("Only the primary action owner can mark this Ready for Closure.","error"); return; }
  openModal("Mark Ready for Closure",`
    ...
    <div id="rfc_ai" style="margin-top:12px"></div>`,
    `<button class="btn sec" onclick="closeModal()">Cancel</button><button class="btn ai-generate-btn" id="rfc_submit" onclick="submitReadyForClosure('${aid}','${rid}','${oid}')">Submit</button>`);
}
function rfcFilePicked(inp){ const el=document.getElementById("rfc_fileName"); if(el) el.textContent=(inp&&inp.files&&inp.files[0])?inp.files[0].name:"No file chosen (optional)"; }
...
Return ONLY a JSON object: {"concrete": true or false, "feedback": "one or two sentences of specific feedback", "questions": ["a short probing question the owner should answer","..."], "suggestion": "an improved, concrete rewrite the owner can adapt to what they actually did — only when not concrete, else empty string. If it contains numbered or sequential steps, put EACH step on its own line separated by a newline (\\n), not packed into one paragraph."}`;
  const raw=await aiGenerate(prompt,"json");
  const d=parseAiJson(raw);
  return { concrete: d.concrete===true||String(d.concrete).toLowerCase()==="true", feedback:d.feedback||"", questions:Array.isArray(d.questions)?d.questions:[], suggestion:d.suggestion||"" };
}
async function submitReadyForClosure(aid,rid,oid){
  const o=findObs(aid,rid,oid); if(!o) return;
  const text=val("rfc_text");
  const aiBox=document.getElementById("rfc_ai");
  if(aiBox) aiBox.innerHTML=""; // clear any previous feedback before checking again
  ...
  if(!verdict.concrete){
    window._rfcSuggestion=verdict.suggestion||"";
    const heading=RFC_HEADINGS[_rfcHeadingIdx%RFC_HEADINGS.length]; _rfcHeadingIdx++;
    if(aiBox) aiBox.innerHTML=`<div class="note" style="border-left:3px solid var(--crit)">
      <div style="font-weight:700;color:var(--crit);margin-bottom:4px">⚠ ${esc(heading)}</div>
      <div class="hint">${esc(verdict.feedback||"Add specifics: exactly what was changed, when, and how it addresses the recommendation.")}</div>
      ${verdict.questions.length?`<ul style="margin:8px 0 0;padding-left:18px;font-size:13px">${verdict.questions.map(q=>`<li>${esc(q)}</li>`).join("")}</ul>`:""}
      ${verdict.suggestion?`<div class="hint" style="margin-top:10px"><b>Suggested wording — edit it to reflect what you actually did:</b></div><div class="note" style="margin-top:4px;background:var(--card);white-space:pre-wrap;line-height:1.5">${esc(verdict.suggestion)}</div><div style="margin-top:6px"><button class="btn sec sm" type="button" onclick="rfcUseSuggestion()">Copy</button></div>`:""}
    </div>`;
    return;
  }
  if(aiBox) aiBox.innerHTML=`<div class="hint" style="color:var(--low)">✓ Looks concrete. Submitting…</div>`;
  await finalizeReadyForClosure(aid,rid,oid,text);
}
async function finalizeReadyForClosure(aid,rid,oid,text){
  const o=findObs(aid,rid,oid); if(!o) return;
  const u=window.AMS_USER||{};
  const fileEl=document.getElementById("rfc_file");
  let ev=null;
  if(fileEl&&fileEl.files&&fileEl.files.length){ ev=await uploadEvidenceFile(oid,fileEl); if(!ev) return; }
  o.ownerResponse=text;
  ...
  emailNotify(uniq(emails),"AuditLens — observation Ready for Closure",`The action owner marked "${o.title}" as Ready for Closure. Sign in to AuditLens to review the response and verify.`);
  logAudit("obs.ready_for_closure","Owner marked Ready for Closure: "+o.title,{observationId:oid});
  save(); closeModal(); render(); modalSuccess("Marked Ready for Closure and sent back to the auditor who raised it for verification.");
}
...
function coFilePicked(inp){ const el=document.getElementById("co_fileName"); if(el) el.textContent=(inp.files&&inp.files[0])?inp.files[0].name:"No file chosen"; }
...
async function enhanceClosureNote(aid,rid,oid,targetId){
  const o=findObs(aid,rid,oid); if(!o) return;
  const ta=document.getElementById(targetId); if(!ta) return;
  const cur=(ta.value||"").trim();
  const prompt=`You are an internal audit manager writing the closure note for an audit observation being signed off. Expand the draft into a clear, professional 2–4 sentence closure note stating what remediation was verified, the evidence relied upon, and confirmation the recommendation is addressed. Keep it factual and concise; do not invent specifics not implied by the inputs.

Observation: ${o.title}
Recommendation: ${o.recommendation||"(none)"}
Owner's closure response: ${o.ownerResponse||"(none)"}
Draft closure note: ${cur||"(empty — draft one from the above)"}

Return plain text only (the closure note).`;
  const done=btnBusy(event&&event.currentTarget,"Enhancing…");
  try{ const text=await aiGenerate(prompt,"text"); if(text) ta.value=text.trim(); }
  catch(e){ toast("Could not enhance the note right now.","error"); }
  finally{ done(); }
}
...
function modalClosureReject(aid,rid,oid,target){
  if(!isHeadUser()){ toast("Only the Head of Audit can do this.","error"); return; }
  const o=findObs(aid,rid,oid); if(!o) return;
...
function submitClosureReject(aid,rid,oid,target){
  if(!isHeadUser()){ toast("Only the Head of Audit can do this.","error"); return; }
  const o=findObs(aid,rid,oid); if(!o) return;
...
function modalHeadClose(aid,rid,oid){
  const o=findObs(aid,rid,oid); if(!o) return;
  openModal("Close observation — Head sign-off",`
    <div class="note" style="margin-bottom:10px"><b>${esc(o.title)}</b></div>
...
async function headVerifyClose(aid,rid,oid){
  ...
  const hcComment=val("hc_comment"); if(hcComment) o.headComment=hcComment;
...
function obsRemediationHTML(o,a,r){
  ...
  return `<div class="obs-notes">
    ...
    ${canPost?`<div class="obs-composer" style="margin-top:12px;padding-top:12px;border-top:1px solid var(--line)">
      ...
      ${nextHint?`<div class="hint" style="margin:2px 0 8px">${esc(nextHint)}</div>`:""}
      ${actions?`<div class="row" style="gap:8px;flex-wrap:wrap">${actions}</div>`:""}
    </div>`:"")}
  </div>`;
}
...
function viewMyFraud(){
  ...
  const hiRes=risks.filter(f=>{ const inh=fraudBand(f.likelihood*f.impact); const res=f.residualOverride||residualBand(inh,f.controlStrength); return res==="Extreme"||res==="High"; }).length;
  let h=`<div class="dash-kpis anim-fade-in" style="grid-template-columns:repeat(4,1fr)">
    ${kpi(hiRes?"bad":"good","Extreme / High",hiRes,"residual fraud risks")}
    ${kpi("base","Total risks",risks.length,"assigned to you")}
    ${kpi(unmit?"warn":"good","Unmitigated actions",unmit,"not yet implemented")}
    ${kpi(acts.length&&impl===acts.length?"good":"accent","Implemented",impl,"of "+acts.length+" prevention actions")}
  </div>`;
  risks.forEach(f=>{
    const inh=fraudBand(f.likelihood*f.impact); const res=f.residualOverride||residualBand(inh,f.controlStrength);
...
function resetAllData(){
  uiConfirm("Delete ALL content — audits, reports, observations, audit universe, fraud risks/plan and process reviews? Your org name and report author are kept. This cannot be undone.",()=>{
    ...
    save(); go("dashboard");
  },{danger:true,title:"Reset all data",confirmLabel:"Delete everything"});
}
...
function btnBusy(btn,label){
  ...
  const lbl = label===undefined ? "Saving…" : label; // pass "" for a spinner-only loader
  btn.disabled=true; btn.innerHTML=`<span class="btn-spin" aria-hidden="true"></span>${lbl?" "+esc(lbl):""}`;
...
function modalWelcome(){
  markOnboarded(); // show the onboarding only once — even if dismissed via the X
...
function tourSteps(){
  if(isActionOwner()) return [
    {title:"Your dashboard",view:"dashboard",body:"The <b>Dashboard</b> highlights what needs your attention — overdue and due-soon actions assigned to your department."},
    {title:"My Observations",view:"myobs",body:"<b>My Observations</b> lists every finding raised against your department. Open one to see the full details."},
    {title:"Respond & upload evidence",view:"myobs",body:"Open an item and use <b>Your response</b> to add a comment, optionally attach a file, then mark it <b>Ready for Closure</b> for Internal Audit to verify."},
  ];
  if(isStaff()) return [
    {title:"Your audits",view:"audits",body:"In <b>Audits & Reports</b>, the “Assigned to me” filter shows the audits you lead. Open one to add reports."},
    {title:"Raise observations",view:"audits",body:"Inside a report, use <b>+ Add observation (AI)</b> to draft a finding, review and edit it, assign an action owner and submit it for Head of Audit approval."},
    {title:"Track remediation",view:"tracker",body:"The <b>Remediation Tracker</b> shows approved observations by expected close. You'll be notified as owners respond."},
  ];
  return [
    {title:"Oversight at a glance",view:"dashboard",body:"The <b>Dashboard</b> rolls up every audit, observation and exposure across the organisation."},
    {title:"Plan & assess",view:"auditra",body:"Use <b>Audit Risk Assessment</b> to build the universe and annual plan, and <b>Fraud Risk</b> for the fraud register."},
    {title:"Approvals",view:"approvals",body:"The <b>Approvals</b> inbox is where you sign off staff-raised observations, status changes and plan completions — the bell and badge flag new requests."},
    {title:"People & departments",view:"settings",body:"In <b>Settings</b>, add departments (each head becomes an action-owner login) and manage user access."},
  ];
}
let _tourIdx=0;
...
function showTourStep(){
  ...
  if(s.view && canAccessView(s.view)) go(s.view); // walk the user to the relevant page
...
function raiseReviewStep(){
  ...
  const o=pr.obs;
  openModal("Review & edit observation",`
    <div class="hint" style="margin-bottom:12px">Review what the AI generated and edit anything before assigning an owner.</div>
    <div class="f3">
      <div><label>Ref</label><input id="rv_ref" value="${esc(o.ref||"")}" placeholder="e.g. 1.1"></div>
      <div><label>Criticality *</label><select id="rv_crit">${CRITS.map(c=>`<option${(o.criticality||"Moderate")===c?" selected":""}>${c}</option>`).join("")}</select></div>
      <div><label>Category / theme</label><input id="rv_cat" value="${esc(o.category||"")}"></div>
    </div>
    <label>Title *</label><input id="rv_title" value="${esc(o.title||"")}">
    <label>Detailed description (condition)</label><textarea id="rv_desc" class="rv-area">${esc(o.description||"")}</textarea>
    <label>Criteria / expectation</label><textarea id="rv_criteria" class="rv-area">${esc(o.criteria||"")}</textarea>
    <label>Impact / risk</label><textarea id="rv_risk" class="rv-area">${esc(o.risk||"")}</textarea>
    <label>Possible root cause</label><textarea id="rv_root" class="rv-area">${esc(o.rootCause||"")}</textarea>
    <label>Recommendation</label><textarea id="rv_rec" class="rv-area">${esc(o.recommendation||"")}</textarea>
    <div id="rv_err" style="margin-top:8px"></div>`,
    `<button class="btn sec" onclick="cancelRaise()">Cancel</button><button class="btn" onclick="raiseReviewNext()">Next: assign owner →</button>`);
  const m=document.getElementById("modal"); if(m) m.classList.add("wide");
}
function raiseReviewNext(){
  const pr=_pendingRaise; if(!pr) return;
  const o=pr.obs;
  const title=val("rv_title"); if(!title){ const e=document.getElementById("rv_err"); if(e) e.innerHTML=`<div class="ai-err">A title is required.</div>`; return; }
  o.ref=val("rv_ref"); o.criticality=val("rv_crit")||"Moderate"; o.category=val("rv_cat");
  o.title=title; o.description=val("rv_desc"); o.criteria=val("rv_criteria"); o.risk=val("rv_risk");
  o.rootCause=val("rv_root"); o.recommendation=val("rv_rec");
  raiseAssignStep();
}
...
function raiseAssignStep(){
  ...
  openModal("Raise observation — assign owner",`
    ...
    <label>Primary action owner (responds &amp; closes) *</label>
    <select id="rz_owner">${ownerOptions(o.ownerUserId||"","— select primary owner —")}</select>
    ...
    <label style="margin-top:8px">Secondary action owner <span class="hint">(oversight only — optional)</span></label>
    <select id="rz_owner2">${ownerOptions(o.secondaryOwnerUserId||"","— none —")}</select>
    <div class="hint" style="margin-top:4px">The secondary owner is notified and can follow the remediation and comment, but only the primary owner responds and marks it Ready for Closure.</div>
...
function raiseObs(){
  ...
  const owner2Id=val("rz_owner2");
  o.secondaryOwnerUserId=(owner2Id&&owner2Id!==ownerId)?owner2Id:"";
  const dept2=departments().find(d=>d.headUserId===o.secondaryOwnerUserId);
  o.secondaryOwner=dept2?dept2.headName:"";
...
/* ============================ MD & EXCO EXECUTIVE ASSURANCE BRIEF ============================ */
function excoMeta(){ const e=DB.exco=DB.exco||{}; e.period=e.period||""; e.headline=e.headline||""; e.commentary=e.commentary||""; e.recipients=e.recipients||""; e.cc=e.cc||""; e.subject=e.subject||""; e.sends=e.sends||[]; return e; }
function parseEmails(s){ return uniq(String(s||"").split(/[;,\s]+/).map(x=>x.trim()).filter(x=>x.includes("@"))); }
function stripTags(h){ return String(h||"").replace(/<[^>]+>/g,"").replace(/&amp;/g,"&"); }
function excoData(){
  const obs=allObs();
  const total=obs.length; const open=obs.filter(o=>o.status!=="Closed"); const closed=total-open.length;
  const remRate=total?Math.round(closed/total*100):0;
  const keyOpen=open.filter(o=>o.criticality==="Critical"||o.criticality==="High");
  const overdue=open.filter(o=>isOverdueObs(o,o._r));
  const keyOverdue=keyOpen.filter(o=>isOverdueObs(o,o._r));
  const repeats=obs.filter(o=>o.isRepeat); const repeatOpen=repeats.filter(o=>o.status!=="Closed");
  const watch=open.filter(o=>{ const d=daysToClose(o,o._r); return d!=null&&d>=0&&d<=14; });
  const themeCount={}; open.forEach(o=>rcThemes(o.rootCause).forEach(t=>{ if(t==="Other / unclassified")return; themeCount[t]=(themeCount[t]||0)+1; }));
  const themes=Object.entries(themeCount).sort((a,b)=>b[1]-a[1]);
  const fr=fraudList().map(f=>{ const inh=fraudBand((f.likelihood||0)*(f.impact||0)); const res=f.residualOverride||residualBand(inh,f.controlStrength); const acts=f.actions||[]; const done=acts.length&&acts.every(a=>a.status==="Implemented"); return {...f,inh,res,mitigated:f.status==="Mitigated"||done}; });
  const unmit=fr.filter(f=>(f.res==="High"||f.res==="Extreme")&&!f.mitigated).sort((a,b)=>BANDS.indexOf(b.res)-BANDS.indexOf(a.res));
  const ext=extList(); const extOpen=ext.filter(f=>f.status!=="Closed"); const extOverdueN=extOpen.filter(extOverdue).length;
  const plan=(DB.auditUniverse||[]).filter(e=>e.includeInPlan);
  const totOcc=plan.reduce((s,e)=>s+unitOccTotal(e),0), doneOcc=plan.reduce((s,e)=>s+unitOccDone(e),0);
  const planPct=totOcc?Math.round(doneOcc/totOcc*100):0;
  const byC=zc(); obs.forEach(o=>byC[o.criticality]++);
  const openByC=zc(); open.forEach(o=>openByC[o.criticality]++);
  const overall=open.length?worstKey(openByC):"—";
  return {obs,total,open,closed,remRate,keyOpen,overdue,keyOverdue,repeats,repeatOpen,watch,themes,fr,unmit,ext,extOpen,extOverdueN,plan,planPct,byC,openByC,overall};
}
function excoMatters(d){
  const m=[];
  if(d.keyOverdue.length) m.push(`${d.keyOverdue.length} critical/high-risk issue${d.keyOverdue.length!==1?"s are":" is"} overdue — management remediation has slipped past the agreed date and needs executive push.`);
  if(d.unmit.length) m.push(`${d.unmit.length} fraud risk${d.unmit.length!==1?"s remain":" remains"} at High/Extreme residual without full mitigation — prioritise the control actions with the risk owners.`);
  if(d.extOverdueN) m.push(`${d.extOverdueN} regulatory/external audit finding${d.extOverdueN!==1?"s are":" is"} past the remediation deadline — regulatory exposure requiring attention.`);
  if(d.repeatOpen.length) m.push(`${d.repeatOpen.length} repeat finding${d.repeatOpen.length!==1?"s recur":" recurs"} from prior audits — issues are not being sustainably fixed at root cause.`);
  if(d.watch.length) m.push(`${d.watch.length} issue${d.watch.length!==1?"s fall":" falls"} due within two weeks — owner follow-through needed to avoid new overdues.`);
  if(d.themes.length && d.themes[0][1]>=3) m.push(`Risk is concentrating in ${d.themes[0][0]} (${d.themes[0][1]} open issues) — a systemic theme for management focus.`);
  if(d.plan.length && d.planPct<60) m.push(`Annual audit plan delivery is at ${d.planPct}% — monitor to ensure planned assurance coverage is achieved.`);
  if(!m.length) m.push(`No matters require escalation this period — remediation is broadly on track, with no High/Extreme fraud or regulatory exposures outstanding.`);
  return m;
}
function excoHero(d,e){
  return `<div class="exhero">
    <div class="row" style="align-items:flex-start;gap:18px">
      <div style="flex:1">
        <div class="lbl">Internal Audit · Executive Assurance Brief</div>
        <h2>${esc(DB.org)}</h2>
        <div class="postu">As at ${e.period?esc(e.period):fmtDate(today0())}</div>
      </div>
      <div style="text-align:right"><div style="font-size:42px;font-weight:800;line-height:1">${d.remRate}%</div><div style="font-size:12px;color:#cfe7de">issues remediated</div></div>
    </div>
...
function excoSections(d,e){
  ...
  const keySorted=d.keyOpen.slice().sort((a,b)=>(isOverdueObs(b,b._r)-isOverdueObs(a,a._r))||(CRITS.indexOf(a.criticality)-CRITS.indexOf(b.criticality))||(((daysToClose(a,a._r))??1e9)-((daysToClose(b,b._r))??1e9)));
  const bySource={}; d.extOpen.forEach(f=>{ bySource[f.source||"Other"]=(bySource[f.source||"Other"]||0)+1; });
  const extSorted=d.extOpen.slice().sort((a,b)=>(extOverdue(b)-extOverdue(a)));
  let h="";
  ...
  h+=`<div class="card"><div class="seclabel">Critical &amp; high-risk issues requiring executive attention</div>
    ${keySorted.length?`<table><thead><tr><th>Criticality</th><th>Issue</th><th>Action owner</th><th>Target close</th><th>Status</th></tr></thead><tbody>`+
      keySorted.slice(0,10).map(o=>{ const ec=effectiveClose(o,o._r); const od=isOverdueObs(o,o._r); return `<tr>
        <td><span class="pill c-${ck(o.criticality)}">${o.criticality}</span>${o.isRepeat?`<div style="margin-top:3px"><span class="pill" style="background:#efe3f7;color:#6b3fa0">↻ repeat</span></div>`:""}</td>
        <td><b>${esc(o.title)}</b><div class="hint">${esc(o._a.name)}${o._a.area?" · "+esc(o._a.area):""}</div></td>
        <td>${esc(o.owner||"—")}</td>
  ...
  h+=`<div class="card"><div class="seclabel">Unmitigated fraud risks — High / Extreme residual</div>
    ${d.unmit.length?`<table><thead><tr><th>Residual</th><th>Fraud scheme</th><th>Category</th><th>Owner</th><th>Status</th></tr></thead><tbody>`+
      d.unmit.slice(0,10).map(f=>`<tr><td><span class="pill" style="background:${hx2rgba(BAND_HEX[f.res],.16)};color:${BAND_HEX[f.res]};font-weight:700">${f.res}</span></td><td><b>${esc(f.scheme)}</b></td><td>${esc(f.category||"—")}</td><td>${esc(f.owner||"—")}</td><td>${esc(f.status||"Identified")}</td></tr>`).join("")+`</tbody></table>`
      :`<div class="hint">No fraud risks at High/Extreme residual outstanding.${fraudList().length?"":" (No fraud risk assessment captured yet.)"}</div>`}
  </div>`;
  h+=`<div class="card"><div class="seclabel">Regulatory &amp; external audit exposure</div>
    ${d.extOpen.length?`<div class="row" style="gap:8px;flex-wrap:wrap;margin-bottom:10px">${Object.entries(bySource).sort((a,b)=>b[1]-a[1]).map(([s,n])=>`<span class="pill" style="background:#eef2f7;color:#334155">${esc(s)} · ${n}</span>`).join("")}</div>
      <table><thead><tr><th>Source</th><th>Finding</th><th>Owner</th><th>Target</th><th>Status</th></tr></thead><tbody>`+
      extSorted.slice(0,8).map(f=>`<tr><td>${esc(f.source||"—")}</td><td><b>${esc(f.title||f.ref||"—")}</b></td><td>${esc(f.owner||"—")}</td><td>${esc(f.targetDate||"—")}${extOverdue(f)?` <span class="pill c-Critical">overdue</span>`:""}</td><td>${esc(f.status||"Open")}</td></tr>`).join("")+`</tbody></table>`
      :`<div class="hint">No open external/regulatory findings.${extList().length?"":" (No external findings captured yet.)"}</div>`}
  </div>`;
  return h;
}
function modalExcoBrief(){
  const e=excoMeta();
  openModal("Executive key message — MD & EXCO",`
    <label>As at / period</label><input id="ex_per" value="${esc(e.period||"")}" placeholder="e.g. As at 30 June 2026 (Q2 2026)">
    <label>Key message (executive headline)</label><textarea id="ex_head" style="min-height:120px" placeholder="One short paragraph: the overall assurance message the MD & EXCO should take away. Use separate lines for numbered points.">${esc(e.headline||"")}</textarea>
    <label>CAE commentary (optional, longer)</label><textarea id="ex_comm" style="min-height:130px" placeholder="Optional supporting narrative — critical issues, themes, fraud, regulatory.">${esc(e.commentary||"")}</textarea>`,
    `<button class="btn sec" onclick="closeModal()">Cancel</button><button class="btn" onclick="saveExcoBrief()">Save</button>`);
}
function saveExcoBrief(){ const e=excoMeta(); e.period=val("ex_per"); e.headline=val("ex_head"); e.commentary=val("ex_comm"); save(); closeModal(); render(); }
function exportExco(){
  const d=excoData(); const e=excoMeta(); const matters=excoMatters(d);
  const keySorted=d.keyOpen.slice().sort((a,b)=>(isOverdueObs(b,b._r)-isOverdueObs(a,a._r))||(CRITS.indexOf(a.criticality)-CRITS.indexOf(b.criticality)));
  const extSorted=d.extOpen.slice().sort((a,b)=>(extOverdue(b)-extOverdue(a)));
  let inner=`<h1>Executive Assurance Brief — MD &amp; Executive Committee</h1>
    <div class="meta">${esc(DB.org)} — Internal Audit${e.period?" · "+esc(e.period):""}</div>
    <div class="note">Remediation rate: <b>${d.remRate}%</b> (${d.closed}/${d.total}) · Open Critical &amp; High: <b>${d.keyOpen.length}</b> (${d.keyOverdue.length} overdue) · Unmitigated fraud risks: <b>${d.unmit.length}</b> · Regulatory findings open: <b>${d.extOpen.length}</b> (${d.extOverdueN} overdue).</div>
    ${e.headline?`<h2>Key Message</h2><div>${esc(e.headline).replace(/\n/g,"<br>")}</div>`:""}
    ${e.commentary?`<h2>Chief Audit Executive's Commentary</h2><div>${esc(e.commentary).replace(/\n/g,"<br>")}</div>`:""}
    <h2>Matters Requiring EXCO Attention</h2><ol>${matters.map(t=>`<li>${esc(t)}</li>`).join("")}</ol>
  ...
  inner+= d.unmit.length?`<table><tr><th>Residual</th><th>Scheme</th><th>Category</th><th>Owner</th><th>Status</th></tr>`+
    d.unmit.map(f=>`<tr><td>${f.res}</td><td>${esc(f.scheme)}</td><td>${esc(f.category||"—")}</td><td>${esc(f.owner||"—")}</td><td>${esc(f.status||"Identified")}</td></tr>`).join("")+`</table>`:`<p>No fraud risks at High/Extreme residual outstanding.</p>`;
  inner+=`<h2>Regulatory &amp; External Audit Exposure</h2>`;
  inner+= d.extOpen.length?`<table><tr><th>Source</th><th>Finding</th><th>Owner</th><th>Target</th><th>Status</th></tr>`+
    extSorted.map(f=>`<tr><td>${esc(f.source||"—")}</td><td>${esc(f.title||f.ref||"—")}</td><td>${esc(f.owner||"—")}</td><td>${esc(f.targetDate||"—")}${extOverdue(f)?" (overdue)":""}</td><td>${esc(f.status||"Open")}</td></tr>`).join("")+`</table>`:`<p>No open external/regulatory findings.</p>`;
  wordDoc("Executive Assurance Brief"+(e.period?" - "+String(e.period).replace(/[^\w \-]/g,""):""),inner);
}
/* --- Email summary + public link --- */
function buildExcoSummaryHTML(d,e){
  const teal="#0d5a47"; const matters=excoMatters(d);
  const row=(l,v,c)=>`<tr><td style="padding:6px 10px;border:1px solid #dbe3ea;font-size:13px;color:#334155">${l}</td><td style="padding:6px 10px;border:1px solid #dbe3ea;font-size:14px;font-weight:700;color:${c||teal};text-align:center">${v}</td></tr>`;
  return `<div style="font-family:Calibri,Arial,sans-serif;color:#1c2733;max-width:720px">
    <div style="background:${teal};color:#ffffff;padding:14px 18px;border-radius:6px">
      ...
      <div style="font-size:19px;font-weight:800;margin-top:3px">${esc(DB.org)}</div>
      <div style="font-size:12.5px;color:#cfe7de;margin-top:4px">As at ${e.period?esc(e.period):fmtDate(today0())} · ${d.remRate}% remediated</div>
    </div>
    ${e.headline?`<p style="font-size:14px;line-height:1.55;margin:14px 2px">${esc(e.headline).replace(/\n/g,"<br>")}</p>`:""}
    <table style="border-collapse:collapse;margin:12px 0;width:100%">
      ${row("Open Critical &amp; High issues",d.keyOpen.length+" ("+d.keyOverdue.length+" overdue)",d.keyOpen.length?"#b00020":teal)}
      ${row("Overdue remediation actions",d.overdue.length,d.overdue.length?"#b00020":teal)}
      ${row("Unmitigated fraud risks (High/Extreme)",d.unmit.length,d.unmit.length?"#b00020":teal)}
      ${row("Regulatory / external findings open",d.extOpen.length+" ("+d.extOverdueN+" overdue)",d.extOpen.length?"#c9a300":teal)}
      ${row("Remediation rate",d.remRate+"% ("+d.closed+"/"+d.total+")",teal)}
    </table>
    <div style="font-size:14px;font-weight:700;color:${teal};margin:10px 2px 6px">Matters requiring EXCO attention</div>
    <ol style="margin:0 0 12px;padding-left:20px">
      ${matters.map(t=>`<li style="margin:6px 0;font-size:13px;line-height:1.5;color:#1c2733">${esc(t)}</li>`).join("")}
    </ol>
  </div>`;
}
function buildExcoEmailText(d,e,link){
  const matters=excoMatters(d);
  const lines=[
    `Internal Audit — Executive Assurance Brief for the MD & Executive Committee`,
    `${DB.org} · As at ${e.period||fmtDate(today0())}`,
    ``,
  ];
  if(e.headline) lines.push(e.headline,"");
  lines.push(
    `Remediation rate: ${d.remRate}% (${d.closed}/${d.total})`,
    `Open Critical & High issues: ${d.keyOpen.length} (${d.keyOverdue.length} overdue)`,
    `Overdue remediation actions: ${d.overdue.length}`,
    `Unmitigated fraud risks (High/Extreme): ${d.unmit.length}`,
    `Regulatory / external findings open: ${d.extOpen.length} (${d.extOverdueN} overdue)`,
    ``,
    `Matters requiring EXCO attention:`,
    ...matters.map((t,i)=>`${i+1}. ${t}`),
    ``,
    `Open the full Executive Assurance Brief here: ${link}`
  );
  return lines.join("\n");
}
function buildExcoSnapshot(d,e){
  const keySorted=d.keyOpen.slice().sort((a,b)=>(isOverdueObs(b,b._r)-isOverdueObs(a,a._r))||(CRITS.indexOf(a.criticality)-CRITS.indexOf(b.criticality)));
  const extSorted=d.extOpen.slice().sort((a,b)=>(extOverdue(b)-extOverdue(a)));
  return {
    org:DB.org||"", period:e.period||fmtDate(today0()), generatedAt:new Date().toISOString(),
    headline:e.headline||"", commentary:e.commentary||"",
    remRate:d.remRate, closed:d.closed, total:d.total,
    kpis:{ keyOpen:d.keyOpen.length, keyOverdue:d.keyOverdue.length, overdue:d.overdue.length, unmit:d.unmit.length, extOpen:d.extOpen.length, extOverdueN:d.extOverdueN, watch:d.watch.length },
    matters:excoMatters(d),
...
function publishExcoBrief(){
  const e=excoMeta(); const d=excoData();
  if(!e.token) e.token=uid()+uid();
  DB.exco.snapshot=buildExcoSnapshot(d,e);
  DB.exco.publishedAt=new Date().toISOString();
  return briefLink(e.token);
}
function copyBriefLink(){
  const link=publishExcoBrief(); // (re)publish the current snapshot so the link is always up to date
  save();
  try{ navigator.clipboard.writeText(link).then(()=>toast("Public link copied — the brief is now published.","success"),()=>toast(link)); }catch(err){ toast(link); }
  render();
}
function modalExcoSend(){
  const e=excoMeta(); const d=excoData();
  openModal("Send brief to MD & EXCO",`
    <p class="hint">The executive summary is emailed to the recipients below, with a link to the full brief. Default recipients come from <b>Settings → MD &amp; EXCO recipients</b>.</p>
    <label>To (MD &amp; EXCO — separate with ; or , )</label><input id="ex_to" value="${esc(e.recipients||"")}" placeholder="md@credicorp.ng; director1@credicorp.ng">
    <label>Cc (optional)</label><input id="ex_cc" value="${esc(e.cc||"")}">
    <label>Subject</label><input id="ex_subj" value="${esc(e.subject||("Executive Assurance Brief — "+(e.period||fmtDate(today0()))))}">
    <div class="note" style="margin-top:12px">Email preview (a link to the full brief is included):</div>
    <div style="border:1px solid var(--line);border-radius:8px;padding:10px;max-height:280px;overflow:auto;background:#fff">${buildExcoSummaryHTML(d,e)}</div>
    <div id="ex_send_err" style="margin-top:8px"></div>`,
    `<button class="btn sec" onclick="closeModal()">Cancel</button><button class="btn" id="ex_send_btn" onclick="sendExcoBrief()">✉ Send now</button>`);
}
async function sendExcoBrief(fromAuto){
  const e=excoMeta();
  if(!fromAuto){ e.recipients=val("ex_to"); e.cc=val("ex_cc"); e.subject=val("ex_subj"); }
  const to=parseEmails(e.recipients); const cc=parseEmails(e.cc);
  if(!to.length){ if(fromAuto) return false; const el=document.getElementById("ex_send_err"); if(el) el.innerHTML=`<div class="ai-err">Add at least one recipient (or set them in Settings → MD &amp; EXCO recipients).</div>`; return false; }
  const btn=document.getElementById("ex_send_btn"); const done=btnBusy(btn,"Sending…");
  const link=publishExcoBrief();
  const d=excoData();
  const subject=e.subject||("Executive Assurance Brief — "+(e.period||fmtDate(today0())));
  const text=buildExcoEmailText(d,e,link);
  let sent=false;
  try{
    const res=await fetch("/api/notify",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({to:uniq([...to,...cc]),subject,text,ctaUrl:link,ctaLabel:"Open the Executive Assurance Brief"})});
    const data=await res.json().catch(()=>({})); sent=!!data.sent;
  }catch(err){}
  e.sends=e.sends||[]; e.sends.unshift({at:new Date().toISOString(),by:(window.AMS_USER&&window.AMS_USER.name)||"auto",to:to.length+cc.length,period:e.period||fmtDate(today0()),link,sent});
  if(e.sends.length>50) e.sends=e.sends.slice(0,50);
  e.lastSentAt=new Date().toISOString();
  logAudit("exco.sent","Executive Assurance Brief "+(fromAuto?"auto-":"")+"sent to MD & EXCO ("+to.length+" recipient(s))",{recipients:to.length,period:e.period,sent});
  save();
  if(done) done();
